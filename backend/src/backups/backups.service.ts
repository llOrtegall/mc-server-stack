...
export async function createBackup(serverId: string): Promise<Backup> {
  ...
  const tarProc = Bun.spawn(['tar', '-czf', tarPath, '-C', config.mcDataPath, serverId], {
    stderr: 'pipe',
  })
...
export async function deleteBackup(backupId: string, serverId: string): Promise<void> {
  ...
  await s3.send(new DeleteObjectCommand({ Bucket: config.r2.bucket, Key: backup.s3_key }))
  await pool.query('DELETE FROM backups WHERE id = $1', [backupId])
}

export async function restoreBackup(backupId: string, serverId: string): Promise<void> {
  const result = await pool.query<{ s3_key: string }>(
    'SELECT s3_key FROM backups WHERE id = $1 AND server_id = $2',
    [backupId, serverId],
  )
