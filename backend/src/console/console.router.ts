...
consoleRouter.get('/logs', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!server?.container_id) throw new AppError(404, 'Server not found or has no container')
...
consoleRouter.post('/command', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const serverId = req.params['id'] as string
    ...
    const response = await consoleService.executeCommand(serverId, body.data.command)
    res.json({ response })
  } catch (err) {
    next(err)
  }
