...
export async function startServer(id: string): Promise<void> {
  ...
  await pool.query(
    "UPDATE servers SET status = 'starting', updated_at = NOW() WHERE id = $1",
    [id],
  );
  try {
    await dockerService.startContainer(server.container_id);
    await pool.query(
      "UPDATE servers SET status = 'running', updated_at = NOW() WHERE id = $1",
      [id],
    );
  } catch (err) {
    await pool.query(
      "UPDATE servers SET status = 'error', updated_at = NOW() WHERE id = $1",
      [id],
    );
    throw err;
  }
...
export async function stopServer(id: string): Promise<void> {
  ...
  resetCounter(id);
  await pool.query(
    "UPDATE servers SET status = 'stopping', updated_at = NOW() WHERE id = $1",
    [id],
  );
